...
class StaticViewsMixin:
    ...
    def _build_guide_view(self) -> None:
        ...
        intro = (
            ...
            "「网络诊断」「子网计算」「交换机配置」「数据库诊断」「ARP安全」「安全诊断」。"
        )
        ttk.Label(
            frm,
            text=intro,
...
六、导出
  ...
  • **ARP 安全**：在本机 **Windows** 上周期性读取 **`arp -a`**，
    对比 **默认网关** 在 ARP 表中的 **MAC** 是否与点击 **「开始」监视后首次成功的观测一致；若发生变化则提示 **疑似网关 ARP 欺骗**
    并在中部给出处置建议（交换机命令示例为通用占位，厂商 CLI 请自行替换）。
...
══════════════════════════════════════
  ...
  • **顶部**：单行展示本机 IP、网关、网段、设备数、异常 IP 数（无分组标题，仅浅色描边框）。
  • **中部**：三栏 **等宽（1:1:1）**——实时日志 / 异常设备详情 / 设备定位建议；各栏标题在面板 **左上角**。
  • **底部**：同一行左侧为 **状态** 说明，右侧为 **开始/停止合一按钮**（未监视时显示「开始」，监视中显示「停止」）以及 **重启**。
...
══════════════════════════════════════
  ...
  • **基线 MAC**：每次点击 **开始** 或 **重启** 后，**第一次** 成功读到网关 MAC 即记为基线；之后不一致则告警。
  • **设备数**：当前网段内在 ARP 表中出现过的 IPv4 数量（粗略统计，不等价于全网终端 census）。
  • **非 Windows**：可能无法可靠解析，启动时会提示；建议在 Windows 桌面运维场景使用。
